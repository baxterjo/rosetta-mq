...
decoder = "json_template"
# Print decoded output to stdout instead of publishing it back to the broker.
...
- an inline table literal carrying the codec's own fields directly (`kind`
  plus that type's fields), for a one-off decoder not worth naming, e.g.
  `decoder = { kind = "utf8" }`.
...
sending `success_output`/`error_output` (below) to different places.
...
topic_filter = "devices/+/raw"
decoder = { kind = "utf8" }
# Equivalent to the defaults, spelled out:
success_output = { publish = { topic = { suffix = "/decoded" } } }
error_output = { publish = { topic = { suffix = "/decode_error" } } }
```
...
`publish`'s `topic` is one of:

| Form       | Example                                | Resulting topic                                                       |
|------------|------------------------------------------|-------------------------------------------------------------------------|
...
`"inherit"` to use the incoming message's own QoS/retain instead.
...
per `[[topic]]` mapping (not registry-wide), so it never mistakes its own
output for new input and decodes itself in a loop, but a legitimate hop into
a *different* mapping's `topic_filter` is processed normally.
